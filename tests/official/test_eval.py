...
class TestEval(mlx_tests.MLXTestCase):
    ...
    def x_test_async_eval_twice(self):
        for _ in range(1000):
            x = mx.array(1) + mx.array(1) + mx.array(1)
            mx.async_eval(x)
            y = x + 1
    ...
    def x_test_async_eval_in_trace(self):
        def fun(x):
            y = x + 1.0
            mx.async_eval(y)
            return mx.exp(y)
    ...
    def x_test_async_eval_into_eval(self):
        x = mx.array(1)
        y = x + 1
        mx.async_eval(y)
        a = y - 10
    ...
    def x_test_async_eval_into_eval_diff_stream(self):
    ...
    def x_test_eval_slow_fast_multi_stream(self):
        x = mx.ones((8000,))
    ...
    def x_test_multi_output_eval_during_transform(self):
    ...
    def x_test_async_eval_with_multiple_streams(self):
        x = mx.array([1.0])
        y = mx.array([1.0])
        a = mx.array([1.0])
        b = mx.array([1.0])
    ...
    def x_test_donation_for_noops(self):
        def fun(x):
            s = x.shape
            for _ in range(10):
                x = mx.abs(x)
    ...
    def x_test_multistream_deadlock(self):
