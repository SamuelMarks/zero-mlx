...
The `ml-framework-snapshots` tool verifies that `zero-mlx` accurately replicates the public API signatures, parameters, and defaults of the real `mlx` framework.

**To run the checker against the current codebase:**

```bash
ml_framework_snapshots check mlx ~/repos/zero-mlx/src \
    --reference-prefix mlx \
    --target-prefix zero_mlx
```

**What it does:**
- It statically analyzes all Python modules in `zero-mlx/src`.
- It maps the target namespace (`zero_mlx.*`) back to the reference framework (`mlx.*`).
...
```bash
ml-switcheroo-ir compliance ~/repos/zero-mlx/src
```
